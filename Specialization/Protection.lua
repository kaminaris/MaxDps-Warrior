...
function Protection:aoe()
    if (CheckSpellCosts(classtable.ThunderBlast, 'ThunderBlast')) and (debuff[classtable.RendDeBuff].remains <= 1) and cooldown[classtable.ThunderBlast].ready then
        return classtable.ThunderBlast
    end
    if (CheckSpellCosts(classtable.ThunderClap, 'ThunderClap')) and (debuff[classtable.RendDeBuff].remains <= 1) and cooldown[classtable.ThunderClap].ready then
        return classtable.ThunderClap
    end
    if (CheckSpellCosts(classtable.ThunderBlast, 'ThunderBlast')) and (buff[classtable.ViolentOutburstBuff].up and targets >= 2 and buff[classtable.AvatarBuff].up and talents[classtable.UnstoppableForce]) and cooldown[classtable.ThunderBlast].ready then
        return classtable.ThunderBlast
    end
    if (CheckSpellCosts(classtable.ThunderClap, 'ThunderClap')) and (buff[classtable.ViolentOutburstBuff].up and targets >= 4 and buff[classtable.AvatarBuff].up and talents[classtable.UnstoppableForce] and talents[classtable.CrashingThunder] or buff[classtable.ViolentOutburstBuff].up and targets >6 and buff[classtable.AvatarBuff].up and talents[classtable.UnstoppableForce]) and cooldown[classtable.ThunderClap].ready then
        return classtable.ThunderClap
    end
    if (CheckSpellCosts(classtable.Revenge, 'Revenge')) and (Rage >= 70 and talents[classtable.SeismicReverberation] and targets >= 3) and cooldown[classtable.Revenge].ready then
        return classtable.Revenge
    end
    if (CheckSpellCosts(classtable.ShieldSlam, 'ShieldSlam')) and (Rage <= 60 or buff[classtable.ViolentOutburstBuff].up and targets <= 4 and talents[classtable.CrashingThunder]) and cooldown[classtable.ShieldSlam].ready then
        return classtable.ShieldSlam
    end
    if (CheckSpellCosts(classtable.ThunderBlast, 'ThunderBlast')) and cooldown[classtable.ThunderBlast].ready then
        return classtable.ThunderBlast
    end
    if (CheckSpellCosts(classtable.ThunderClap, 'ThunderClap')) and cooldown[classtable.ThunderClap].ready then
        return classtable.ThunderClap
    end
    if (CheckSpellCosts(classtable.Revenge, 'Revenge')) and (Rage >= 30 or Rage >= 40 and talents[classtable.BarbaricTraining]) and cooldown[classtable.Revenge].ready then
        return classtable.Revenge
    end
end
function Protection:generic()
    if (CheckSpellCosts(classtable.ThunderBlast, 'ThunderBlast')) and (( buff[classtable.ThunderBlastBuff].count == 2 and buff[classtable.BurstofPowerBuff].count <= 1 and buff[classtable.AvatarBuff].up and talents[classtable.UnstoppableForce] ) or Rage <= 70 and talents[classtable.Demolish]) and cooldown[classtable.ThunderBlast].ready then
        return classtable.ThunderBlast
    end
    if (CheckSpellCosts(classtable.ShieldSlam, 'ShieldSlam')) and (( buff[classtable.BurstofPowerBuff].count == 2 and buff[classtable.ThunderBlastBuff].count <= 1 or buff[classtable.ViolentOutburstBuff].up ) or Rage <= 70 and talents[classtable.Demolish]) and cooldown[classtable.ShieldSlam].ready then
        return classtable.ShieldSlam
    end
    ...
    if (CheckSpellCosts(classtable.ShieldSlam, 'ShieldSlam')) and cooldown[classtable.ShieldSlam].ready then
        return classtable.ShieldSlam
    end
    if (CheckSpellCosts(classtable.ThunderBlast, 'ThunderBlast')) and (debuff[classtable.RendDeBuff].remains <= 2 and not buff[classtable.ViolentOutburstBuff].up) and cooldown[classtable.ThunderBlast].ready then
        return classtable.ThunderBlast
    end
    if (CheckSpellCosts(classtable.ThunderClap, 'ThunderClap')) and (debuff[classtable.RendDeBuff].remains <= 2 and not buff[classtable.ViolentOutburstBuff].up) and cooldown[classtable.ThunderClap].ready then
        return classtable.ThunderClap
    end
    if (CheckSpellCosts(classtable.ThunderBlast, 'ThunderBlast')) and (( targets >1 or cooldown[classtable.ShieldSlam].ready==false and not buff[classtable.ViolentOutburstBuff].up )) and cooldown[classtable.ThunderBlast].ready then
        return classtable.ThunderBlast
    end
    if (CheckSpellCosts(classtable.ThunderClap, 'ThunderClap')) and (( targets >1 or cooldown[classtable.ShieldSlam].ready==false and not buff[classtable.ViolentOutburstBuff].up )) and cooldown[classtable.ThunderClap].ready then
        return classtable.ThunderClap
    end
    if (CheckSpellCosts(classtable.Revenge, 'Revenge')) and (( Rage >= 80 and targetHP >20 or buff[classtable.RevengeBuff].up and targetHP <= 20 and Rage <= 18 and cooldown[classtable.ShieldSlam].ready==false or buff[classtable.RevengeBuff].up and targetHP >20 ) or ( Rage >= 80 and targetHP >35 or buff[classtable.RevengeBuff].up and targetHP <= 35 and Rage <= 18 and cooldown[classtable.ShieldSlam].ready==false or buff[classtable.RevengeBuff].up and targetHP >35 ) and talents[classtable.Massacre]) and cooldown[classtable.Revenge].ready then
        return classtable.Revenge
    end
    if (CheckSpellCosts(classtable.Execute, 'Execute')) and cooldown[classtable.Execute].ready then
        return classtable.Execute
    end
    if (CheckSpellCosts(classtable.Revenge, 'Revenge')) and (targetHP >20) and cooldown[classtable.Revenge].ready then
        return classtable.Revenge
    end
    if (CheckSpellCosts(classtable.ThunderBlast, 'ThunderBlast')) and (( targets >= 1 or cooldown[classtable.ShieldSlam].ready==false and buff[classtable.ViolentOutburstBuff].up )) and cooldown[classtable.ThunderBlast].ready then
        return classtable.ThunderBlast
    end
    if (CheckSpellCosts(classtable.ThunderClap, 'ThunderClap')) and (( targets >= 1 or cooldown[classtable.ShieldSlam].ready==false and buff[classtable.ViolentOutburstBuff].up )) and cooldown[classtable.ThunderClap].ready then
        return classtable.ThunderClap
    end
    if (CheckSpellCosts(classtable.Devastate, 'Devastate')) and cooldown[classtable.Devastate].ready then
        return classtable.Devastate
    end
end

function Protection:callaction()
    if (CheckSpellCosts(classtable.Pummel, 'Pummel')) and cooldown[classtable.Pummel].ready then
    ...
    if (CheckSpellCosts(classtable.Charge, 'Charge')) and (timeInCombat == 0 or (LibRangeCheck and LibRangeCheck:GetRange('target', true) or 0) >8) and cooldown[classtable.Charge].ready then
    ...
    if (CheckSpellCosts(classtable.DemoralizingShout, 'DemoralizingShout')) and (talents[classtable.BoomingVoice]) and cooldown[classtable.DemoralizingShout].ready then
    ...
    if (CheckSpellCosts(classtable.Demolish, 'Demolish')) and (buff[classtable.ColossalMightBuff].count >= 3) and cooldown[classtable.Demolish].ready then
    ...
    if (CheckSpellCosts(classtable.Shockwave, 'Shockwave')) and (talents[classtable.RumblingEarth] and targets >= 3) and cooldown[classtable.Shockwave].ready then
        MaxDps:GlowCooldown(classtable.Shockwave, cooldown[classtable.Shockwave].ready)
    end
    if (CheckSpellCosts(classtable.ShieldCharge, 'ShieldCharge')) and cooldown[classtable.ShieldCharge].ready then
        return classtable.ShieldCharge
    end
    if (CheckSpellCosts(classtable.ShieldBlock, 'ShieldBlock')) and (buff[classtable.ShieldBlockBuff].duration <= 10) and cooldown[classtable.ShieldBlock].ready then
